...
import Data.ByteString.Lazy as Y
import Language.PureScript.Names ( runIdent)
import Data.Aeson qualified as A
import Language.PureScript.Pretty.Types ( prettyPrintType)
import Language.PureScript.Types (returnType)
...
-- compiler.
readExternsFile :: (MonadIO m, MonadError MultipleErrors m) => FilePath -> m (Maybe Language.PureScript.Externs.ExternsFile)
readExternsFile path = do
  mexterns <- readCborFile path
  return $ do
    externs <- mexterns
    guard $ Language.PureScript.Externs.externsIsCurrentVersion externs
    return externs
...
writeCborType ::  (MonadIO m, MonadError MultipleErrors m) =>  FilePath -> Language.PureScript.Externs.ExternsFile -> m ()
writeCborType filePath value = do
  let createJson a b = A.object [ "fnName" A..= a, "type" A..= b]
  let export = Prelude.map (\case (Language.PureScript.Externs.EDValue name e ) -> createJson (runIdent name)  (Text.pack (prettyPrintType maxBound $ returnType e)); _ -> "") (efDeclarations value)
  makeIO ("write Cbor  JSON file: " <> Text.pack filePath) $ Y.writeFile filePath (encode (toJSON export))
