...
import Data.ByteString.Lazy as BB
...
readCborFile :: (MonadIO m, MonadError MultipleErrors m) => Aeson.FromJSON a => FilePath -> m (Maybe a)
readCborFile path =
  makeIO ("read Binary file: " <> Text.pack path) (readCborFileIO path)

readCborFileIO :: Aeson.FromJSON a => FilePath -> IO (Maybe a)
...
writeCborFile :: (MonadIO m, MonadError MultipleErrors m) => Aeson.ToJSON a => FilePath -> a -> m ()
writeCborFile path value =
  makeIO ("write Cbor file: " <> Text.pack path) (writeCborFileIO path value)

writeCborFileIO :: Aeson.ToJSON a => FilePath -> a -> IO ()
writeCborFileIO path value = do
  createParentDirectory path
  BB.writeFile path (Aeson.encode $ Aeson.toJSON value)
