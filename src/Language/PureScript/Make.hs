...
make' MakeOptions{..} ma@MakeActions{..} ms = do
  ...
  (_, directedGraph) <- sortModules Transitive (moduleSignature . CST.resPartial) ms
  (buildPlan, newCacheDb) <- BuildPlan.construct opts ma cacheDb (sorted, graph, directedGraph)
